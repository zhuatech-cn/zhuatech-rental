...
const title='知华设备租赁管理', subtitle='从设备档期、租赁履约到退租结算';
const token=ref(sessionStorage.getItem('rental-session')||''),user=ref(null),catalog=ref(null),dashboard=ref(null);
...
async function request(path,{method='GET',body,key,raw=false}={}){
 ...
 if(!r.ok){let m='请求失败';try{m=(await r.json()).message||m;}catch{}if(r.status===401){token.value='';user.value=null;sessionStorage.removeItem('rental-session');}throw new Error(m);}
...
async function login(){await guarded(async()=>{const r=await request('/auth/login',{method:'POST',body:{username:username.value,password:password.value}});token.value=r.token;user.value=r.user;sessionStorage.setItem('rental-session',r.token);password.value='';await load();});}
async function logout(){await guarded(async()=>{await request('/auth/logout',{method:'POST',body:{}});token.value='';user.value=null;sessionStorage.removeItem('rental-session');});}
...
async function exportCsv(){await guarded(async()=>{const blob=await request('/export/'+section.value,{raw:true});download(blob,'rental-'+section.value+'.csv');});}
...
<template>
 ...
 <div v-else class="app-shell" :style="{'--accent':catalog?.accent}">
  <aside class="sidebar">
   <div class="brand"><span class="brand-mark">知</span><div><strong>{{title.replace('知华','')}}</strong><small>ZHUATECH · RENTAL</small></div></div>
